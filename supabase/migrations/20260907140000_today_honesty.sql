-- Pass 2.5 — Today-screen honesty (2026-09-07).
--
-- Owner dogfooding after the Pass 2 deploy found:
--   1. "HRV -1.2" read as a broken number; it is the z-score (deviation
--      from the personal baseline in spread units) but no raw value is
--      shown anywhere. The brief now carries today's measured values as an
--      additive `today_raw` object inside `computed` so the driver rows can
--      show "38 ms - -1.2" instead of a bare z-score.
--   2. Brief schema_version 1.3 -> 1.4 (additive key; no renames/removals).
--
-- Everything else in compute_daily_metrics / get_my_daily_brief is carried
-- forward verbatim from 20260907120000_math_honesty (ln-domain HRV, load
-- gates, sleep sub honesty, plausibility bands, future-date guard).
...
begin
  if v_user_id is null then
    return jsonb_build_object('schema_version', '1.4',
