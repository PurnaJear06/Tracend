-- fix: "column pw.target_day_of_week does not exist" in prepare_coach_chat_v4 recovery branch
-- the planned_workouts table has preferred_weekday (smallint 1-7), not target_day_of_week
-- target_week_of_block also does not exist — use 1 as placeholder until the column is added
-- this fix replaces the v4 function body with corrected column references
-- v5 wraps v4 and does not have its own training_week_structure subquery
...
begin
  ...
  if ctx_kind = 'daily_action' then
    ...
    c := c || jsonb_build_object(
      'today_confirmed_meals', (select coalesce(jsonb_agg(
        jsonb_build_object('created_at', created_at, 'local_date', local_date, 'foods', foods)
        order by created_at desc), '[]'::jsonb) from (
    ...
    c := c || jsonb_build_object(
      ...
      'training_week_structure', (select jsonb_build_object(
        ...
        'planned_workouts', (select coalesce(jsonb_agg(jsonb_build_object(
          'name', pw.name, 'target_day', pw.preferred_weekday, 'target_week', 1
...
revoke all on function public.prepare_coach_chat_v4(uuid,uuid,text,text,uuid,text)
  from public, anon, authenticated;
grant execute on function public.prepare_coach_chat_v4(uuid,uuid,text,text,uuid,text)
  to service_role;
