-- Pass 3 — Baseline dynamics (2026-09-07).
--
-- Two review gaps fixed in one additive migration:
--
-- 3a. Spread as EWMA + floorSpread (review P2 #10, noop parity):
--     the stored spread was the static full-history 1.4826·MAD, so a
--     metric whose day-to-day noise shrank kept being z-scored against
--     its whole noisy past, and one whose noise grew kept being scored on
--     an ancient calm window. The stored spread is now a 21-day-half-life
--     EWMA over per-observation |deviation| (winsorized input, per-metric
--     floor) — the same window the 14-day center half-life lives in, one
--     step wider for stability. Static MAD stays only where a static
--     scale is the right tool: the Winsor bounds for the fold itself.
--     floorSpread per metric (noop values): hrv ln(ms) 0.05, rhr 2 bpm,
--     sleep 15 min, weight 0.5 kg, resp 0.5 bpm — a spread below the
--     floor would amplify noise into dramatic z-scores on near-identical
--     histories.
--
-- 3b. Staleness tracking (review P2 #11): user_baselines.last_observation_date
--     now stamps the TRUE newest observation date (it previously recorded
--     the compute's target_date, so a user syncing on 09-08 with newest data
--     from 08-26 read as fresh). The brief's per-metric baseline objects
--     carry additive last_obs_date + age_days (0 = today) so a stale number
--     can never silently masquerade as current — the Aug-26-presented-as-
--     today class. Display and AI-context (Pass 4) consume these fields.
--     z-scoring policy stays as-is: a stale-but-valid baseline still
--     z-scores (that is what "personal baseline" means), the fields make
--     the staleness VISIBLE. vNext may gate on it.
--
-- Additive only: two new SQL functions, create-or-replace of the three
-- engine functions, brief 1.4 -> 1.5 (additive keys), scoring stays 2.2.
-- immediately after 20260907140000 (today_honesty).

-- Per-metric spread floor (noop floorSpread values).
create function public.baseline_floor_spread(metric_name_in text)
returns numeric language sql immutable as $$
  select case metric_name_in
    when 'hrv_sdnn_ms' then 0.05  -- ln(ms) domain
    when 'resting_hr_bpm' then 2.0
    when 'sleep_minutes' then 15.0
    when 'weight_kg' then 0.5
    when 'resp_rate_bpm' then 0.5
    else 0.0
  end
$$;

revoke all on function public.baseline_floor_spread(text) from public, anon, authenticated;


-- One-time backfill: existing rows keep their values; the next fold
-- re-derives spread via the EWMA and stamps true dates. Nothing else
-- needed — the stored spread is compatible in kind (a spread), and
-- brief consumers gain the new fields on next compute.
  ...
  loop
    ...
    case _metric_name
      when 'hrv_sdnn_ms' then
        ...
        into values_arr, raw_arr, dates_arr
      ...
      when 'sleep_minutes' then
        select array_agg(sleep_minutes order by local_date),
               array_agg(local_date order by local_date)
        into values_arr, dates_arr
        from public.daily_health_summaries
        where user_id = target_user_id
        ...
        raw_arr := values_arr;
      when 'weight_kg' then
        select array_agg(w.weight_kg order by w.measured_on),
               array_agg(w.measured_on order by w.measured_on)
        into values_arr, dates_arr
        from (
      ...
      when 'resp_rate_bpm' then
        select array_agg(respiratory_rate_bpm order by local_date),
               array_agg(local_date order by local_date)
        into values_arr, dates_arr
        from public.daily_health_summaries
        where user_id = target_user_id
    ...
    if values_arr is null or array_length(values_arr, 1) < 3 then
      insert into public.user_baselines (
        ...
        public.baseline_floor_spread(_metric_name),
        coalesce(array_length(values_arr, 1), 0),
        'cold_start',
        -- Zero observations = never observed: null date, so the brief
        -- reports null age_days (never observed is not "observed today").
        -- One or two observations still stamp the true observation date.
        case when array_length(values_arr, 1) is null
          then null
          else dates_arr[array_length(values_arr, 1)] end
      )
    ...
    -- Winsor bounds use the full-history scale so early bands are not
    -- pathological on tiny histories; the SPREAD STORED as the baseline is
    -- the recent-window tracker below (Pass 3a).
    spread := 1.4826 * coalesce(mad, 0);
    floor_spread := public.baseline_floor_spread(_metric_name);
    spread := greatest(floor_spread, spread);
    ...
    for i in 2..n_obs loop
      ...
      -- Spread tracks |deviation| with a 21-day half-life (noop):
      -- lambda = 1 - 0.5^(1/21) ~= 0.0323; 0.9670/0.0330 are the rounded
      -- complements. Input is the raw deviation from the running center
      -- (the same value the Winsor bounds clamp for the center update), the
      -- floor keeps near-identical histories from amplifying noise into
      -- dramatic z-scores.
      spread_ewma := greatest(
        floor_spread,
        0.9670 * spread_ewma + 0.0330 * abs(values_arr[i] - ewma)
      );
    ...
    newest_obs_date := dates_arr[n_obs];
    insert into public.user_baselines (
      user_id, metric_name, baseline_value, spread, n_observations,
      confidence, last_observation_date
    ) values (
      target_user_id, _metric_name, baseline, spread, n_obs,
      confidence, newest_obs_date
    )
    on conflict (user_id, metric_name) do update set
      baseline_value = excluded.baseline_value,
      spread = excluded.spread,
...
begin
    ...
    loop
      baseline_jsonb := baseline_jsonb || jsonb_build_object(
        ...
        jsonb_build_object(
          ...
          'last_obs_date', baseline.last_observation_date,
          'age_days', case when baseline.last_observation_date is null
            then null
            else target_date - baseline.last_observation_date end
        )
      );
    end loop;
  ...
  if today_health.hrv_value_ms is not null
    ...
    if found and baseline.spread > 0 and baseline.n_observations >= 3 then
  ...
  return jsonb_build_object(
    ...
    'scores', scores_jsonb,
    'today_raw', jsonb_build_object(
      'hrv_ms', today_health.hrv_value_ms,
      'resting_hr_bpm', today_health.resting_heart_rate_bpm,
      'sleep_minutes', today_health.sleep_minutes,
      'resp_rate_bpm', today_health.respiratory_rate_bpm,
