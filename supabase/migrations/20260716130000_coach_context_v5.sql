create table public.physique_analyses (
  ...
  baseline_photo_set_id uuid not null,
  current_photo_set_id uuid not null,
  version integer not null default 1 check (version >= 1),
  provider text check (length(provider) <= 80),
  model text check (length(model) <= 80),
  qualitative_observations text check (length(qualitative_observations) <= 2000),
  development_priorities text check (length(development_priorities) <= 500),
  body_fat_range text check (length(body_fat_range) <= 100),
  confidence text check (confidence in ('low','medium','high')),
  limitations text check (length(limitations) <= 500),
  validation_state text not null default 'pending',
  created_at timestamptz not null default now(),
  foreign key(baseline_photo_set_id, user_id) references public.progress_photo_sets(id, user_id),
  foreign key(current_photo_set_id, user_id) references public.progress_photo_sets(id, user_id),
  unique(id, user_id)
);

create index physique_analyses_user_date on public.physique_analyses(user_id, created_at desc);

alter table public.physique_analyses enable row level security;
alter table public.physique_analyses force row level security;
create policy physique_analyses_own_read on public.physique_analyses
  for select to authenticated using (user_id = (select auth.uid()));

revoke all on public.physique_analyses from anon, authenticated;
grant select on public.physique_analyses to authenticated;
...
begin
  ...
  c := c || jsonb_build_object('schema_version', '3.0', 'context_kind', ctx_kind);
  ...
  if ctx_kind = 'daily_action' then
    ...
    c := c || jsonb_build_object(
      'session_trends', (select coalesce(jsonb_agg(trend order by local_date desc), '[]'::jsonb) from (
        select s.local_date, jsonb_build_object(
        ...
        join public.planned_workouts w on w.id = s.planned_workout_id and w.user_id = s.user_id
        where s.user_id = target_user_id and s.state = 'completed'
        order by s.local_date desc limit 12
      ) sessions),
      'plan_proposals', (select coalesce(jsonb_agg(jsonb_build_object(
        ...
        'rationale', coalesce(rationale, ''), 'confidence', confidence,
        'expected_benefit', expected_benefit, 'effective_date', effective_date
      ) order by created_at desc), '[]'::jsonb)
      from public.change_proposals where user_id = target_user_id limit 5),
      'workout_reconciliations', (select coalesce(jsonb_agg(jsonb_build_object(
        'evidence_id', 'HEALTH.WORKOUT.' || h.id, 'status', r.status,
        'confidence', r.confidence, 'activity_type', h.activity_type,
        'duration_difference_seconds', r.duration_difference_seconds
      ) order by h.started_at desc), '[]'::jsonb)
      ...
      where r.user_id = target_user_id limit 5),
      'eight_week_measurement_delta', (select jsonb_build_object(
        'earliest', (select jsonb_build_object('measured_on', measured_on, 'weight_kg', weight_kg, 'waist_cm', waist_cm)
          from public.body_measurements where user_id = target_user_id order by measured_on asc limit 1),
        'latest', (select jsonb_build_object('measured_on', measured_on, 'weight_kg', weight_kg, 'waist_cm', waist_cm)
          from public.body_measurements where user_id = target_user_id order by measured_on desc limit 1)
      )),
      'seven_day_healthkit_trend', (select jsonb_build_object(
        'avg_resting_hr', (select round(avg(resting_heart_rate_bpm)::numeric, 1)
          from public.daily_health_summaries where user_id = target_user_id
            and local_date between coaching_date - 6 and coaching_date and resting_heart_rate_bpm is not null),
        'avg_hrv_sdnn', (select round(avg(hrv_value_ms)::numeric, 1)
          from public.daily_health_summaries where user_id = target_user_id
            and local_date between coaching_date - 6 and coaching_date and hrv_value_ms is not null),
        'avg_sleep_minutes', (select round(avg(sleep_minutes)::numeric, 0)
          from public.daily_health_summaries where user_id = target_user_id
            and local_date between coaching_date - 6 and coaching_date and sleep_minutes is not null)
      )),
      'nutrition_adherence', jsonb_build_object(
        ...
        'schedule_slot_compliance', (select jsonb_build_object(
          'scheduled_slots', (select count(*) from public.nutrition_schedule_items i
          ...
          'matched_slots_today', (select count(distinct m.nutrition_schedule_item_id)
            from public.meals m where m.user_id = target_user_id
              and m.status = 'confirmed' and m.local_date = coaching_date
              and m.nutrition_schedule_item_id is not null)
        ))
      ),
      'data_quality', jsonb_build_object(
        'training_logging_coverage', (select coalesce(avg(logging_completeness), 0)
        ...
        'last_photo_set', (select max(captured_on)
          from public.progress_photo_sets where user_id = target_user_id and status = 'complete'),
    ...
    c := c || jsonb_build_object(
      'evidence_freshness', jsonb_build_object(
        ...
        'last_photo_set', (select max(captured_on) from public.progress_photo_sets where user_id = target_user_id and status = 'complete')
      ),
      'data_quality', jsonb_build_object(
        'training_logging_coverage', (select coalesce(avg(logging_completeness), 0)
          from public.workout_sessions where user_id = target_user_id and state = 'completed' and local_date >= coaching_date - 27),
        'last_health_sync', (select max(last_synced_at) from public.daily_health_summaries where user_id = target_user_id),
        'last_confirmed_meal', (select max(created_at) from public.meals where user_id = target_user_id and status = 'confirmed'),
        'photo_sets_completed', (select count(*) from public.progress_photo_sets where user_id = target_user_id and status = 'complete'),
        'has_physique_analysis', exists(select 1 from public.physique_analyses where user_id = target_user_id)
      ),
        ...
        'avg_daily_protein_g', (select round(coalesce(avg(day_pro), 0)::numeric, 0)
          from (select m.local_date, coalesce(sum(mi.protein_g), 0) day_pro
            ...
            where m.user_id = target_user_id and m.status = 'confirmed'
              and m.local_date between coaching_date - 6 and coaching_date
            group by m.local_date) days),
        'avg_daily_carbohydrate_g', (select round(coalesce(avg(day_carb), 0)::numeric, 0)
          from (select m.local_date, coalesce(sum(mi.carbohydrate_g), 0) day_carb
            from public.meals m join public.meal_items mi on mi.meal_id = m.id and mi.user_id = m.user_id
            where m.user_id = target_user_id and m.status = 'confirmed'
              and m.local_date between coaching_date - 6 and coaching_date
            group by m.local_date) days),
        'avg_daily_fat_g', (select round(coalesce(avg(day_fat), 0)::numeric, 0)
          from (select m.local_date, coalesce(sum(mi.fat_g), 0) day_fat
            from public.meals m join public.meal_items mi on mi.meal_id = m.id and mi.user_id = m.user_id
            where m.user_id = target_user_id and m.status = 'confirmed'
              and m.local_date between coaching_date - 6 and coaching_date
            group by m.local_date) days),
        'days_with_meals', (select count(distinct m.local_date)
          from public.meals m where m.user_id = target_user_id and m.status = 'confirmed'
            and m.local_date between coaching_date - 6 and coaching_date)
      )),
  ...
  values(target_user_id, target_thread_id, '3.0', 'chat', coaching_date, c, checksum) returning id into snapshot_id;
